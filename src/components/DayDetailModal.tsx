...
function DayDetailModalComponent({ isOpen, onClose, dailyData, categories }: DayDetailModalProps) {
    // Close on escape key
    useEffect(() => {
    ...
    return (
        <div
            className="fixed top-75 inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
            onClick={onClose}
        >
            <div
...
// FIX: Wrap with React.memo and deeply check structural updates
export default memo(DayDetailModalComponent, (prevProps, nextProps) => {
    // If the visibility status changes, re-render
    if (prevProps.isOpen !== nextProps.isOpen) return false

    // If categories metadata tracking configuration updates, re-render
    if (prevProps.categories.length !== nextProps.categories.length) return false

    // Deep compare the daily stats dataset string
    const prevDataString = JSON.stringify(prevProps.dailyData)
    const nextDataString = JSON.stringify(nextProps.dailyData)

    // True means values match perfectly -> block the 1-second render loop entirely!
