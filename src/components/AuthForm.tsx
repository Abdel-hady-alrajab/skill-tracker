...
export default function AuthForm() {
  ...
  async function handleGoogle() {
    ...
    // Use env URL if available, otherwise default to location.origin for local development
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || window.location.origin
    const redirectUrl = `${siteUrl.endsWith('/') ? siteUrl.slice(0, -1) : siteUrl}/auth/callback`
